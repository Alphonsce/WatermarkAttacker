...
class StableSignatureWatermarker(Watermarker):
    def __init__(
        self, stable_diffusion_root_path, msg_extractor, script, key="111010110101000001010111010011010100010000100111"
    ):
    ...
    def encode(self, img_path, output_dir, prompt=""):
        command = [
            ...
            prompt,
            "--ckpt",
            os.path.join(self.stable_diffusion_root_path, "checkpoints/v2-1_512-ema-pruned.ckpt"),
            "--config",
            os.path.join(self.stable_diffusion_root_path, "configs/stable-diffusion/v2-inference.yaml"),
            "--H",
            "512",
            "--W",
            "512",
